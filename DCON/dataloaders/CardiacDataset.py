...
BASEDIR = os.path.join(DATA_ROOT, 'cardiac')
...
def get_normalize_op(modality, fids):
    def get_MR_statistics(scan_fids):
        total_val = 0
        n_pix = 0
    ...
    mr_mean, mr_std = get_MR_statistics(fids)

    return NormalizeOp(mr_mean, mr_std)
...
class CardiacDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains: list,  idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, extern_norm_fn = None, opt=None):
        ...
        for _domain in self.domains:
            self.img_pids[_domain] = sorted([ fid.split("_")[-1].split(".nii.gz")[0] for fid in glob.glob(self._base_dir + "/processed/" +  _domain  + "/image_*.nii.gz") ], key = lambda x: int(x))
    ...
    def __search_samples(self, scan_ids):
        ...
        for _domain, id_list in scan_ids.items():
            ...
            for curr_id in id_list:
                ...
                _img_fid = os.path.join(self._base_dir, 'processed', _domain  ,f'image_{curr_id}.nii.gz')
                _lb_fid  = os.path.join(self._base_dir, 'processed', _domain, f'label_{curr_id}.nii.gz')
...
def get_training(modality, opt):
    print("get_train cardiac:",modality)
...
def get_trval(modality, opt):
    ...
    return CardiacDataset(mode = 'trval',transforms = None,domains = modality,base_dir = BASEDIR,extern_norm_fn = None,opt=opt)

def get_trtest(modality, opt):
    print("get_trtest cardiac:",modality)
    return CardiacDataset(mode = 'trtest',transforms = None,domains = modality,base_dir = BASEDIR,extern_norm_fn = None,opt=opt)

def get_test(modality, opt):
     print("get_test cardiac:",modality)
     return CardiacDataset(mode = 'test',transforms = None,domains = modality,base_dir = BASEDIR,extern_norm_fn = None,opt=opt)
