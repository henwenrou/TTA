...
The current DCON implementation contains the PPM/source-supervised part of
SM-PPM. No explicit SM style-mixing module is present in this file, so the
``sm_ce`` ablation is reported as unavailable rather than faked.
"""
...
class SMPPMAdapter:
    ...
    def __init__(
        ...
        if self.ablation_mode == "sm_ce" and not STYLE_MIXING_AVAILABLE:
            logger.error(
                "[SM-PPM Ablation Mode] sm_ce unavailable: current DCON "
                "tta_smppm.py has no explicit SM style-mixing implementation."
            )
            raise NotImplementedError(
                "sm_ce is unavailable because current DCON tta_smppm.py does "
                "not implement SM style mixing."
            )
        ...
        logger.info(
            ...
            self.steps,
            "ce_only" if self.ablation_mode in {"source_ce_only", "ppm_ce"} else (
                "target_entropy_plus_proto_compact" if self.uses_target_only_loss else "original_dcon_segmentation_criterion"
            ),
        )
        if not STYLE_MIXING_AVAILABLE:
            logger.info(
                "SM style mixing unavailable in current DCON tta_smppm.py; "
                "full/ppm_ce use the existing PPM + source-supervised path."
            )
    ...
    @torch.enable_grad()
    def _adapt_one_source_batch(self, source_img, source_label, prototypes=None):
        ...
        if self.ablation_mode in {"source_ce_only", "ppm_ce"}:
