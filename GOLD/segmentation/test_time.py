...
from methods.gold import GOLDSeg
...
def main(description):
    ...
    elif cfg.MODEL.ADAPTATION == "gold":
        model = setup_gold(base_model, device)
    else:
...
def setup_gold(model, device):
    model.to(device)
    optimizer = optim.SGD([p for p in model.parameters() if p.requires_grad], lr=cfg.OPTIM.LR, momentum=cfg.OPTIM.MOMENTUM, weight_decay=cfg.OPTIM.WD)
    gold_model = GOLDSeg(
        model=model,
        optimizer=optimizer,
        crop_size=cfg.SOURCE.CROP_SIZE,
        ...
        rank=cfg.GOLD.RANK,
        tau=cfg.GOLD.TAU,
        alpha=cfg.GOLD.ALPHA,
        T_eig=cfg.GOLD.T_EIG,
        mt=cfg.GOLD.MT,
        device=device,
        s_lr=cfg.GOLD.S_LR,
        s_init_scale=cfg.GOLD.S_INIT_SCALE,
        s_clip=cfg.GOLD.S_CLIP,
        adapter_scale=cfg.GOLD.ADAPTER_SCALE,
        max_pixels_per_batch=cfg.GOLD.MAX_PIXELS_PER_BATCH,
        min_pixels_per_batch=cfg.GOLD.MIN_PIXELS_PER_BATCH,
        n_augmentations=cfg.GOLD.N_AUGMENTATIONS,
        rst_m=cfg.GOLD.RST_M,
        ap=cfg.GOLD.AP,
    )
    return gold_model
