...
import math
import itertools
from .location_scale_augmentation import ClassConditionalAffineCLP, LocationScaleAugmentation
...
class AbdominalDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains: list,  idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, extern_norm_fn = None, location_scale=False, local_aug_type='affine_clp'):
        """
        Args:
        ...
        self.local_aug_type = local_aug_type
        ...
        if location_scale:
            print(f'Applying Location Scale Augmentation on {mode} split with local_aug_type={self.local_aug_type}')
            self.location_scale = LocationScaleAugmentation(vrange=(0.,1.), background_threshold=0.01)
            self.affine_clp = ClassConditionalAffineCLP(vrange=(0., 1.))
        else:
            self.location_scale = None
            self.affine_clp = None
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            if self.location_scale is not None:
                ...
                if self.local_aug_type in ['affine_clp', 'clp']:
                    LLA = self.affine_clp(img.copy(), lb.astype(np.int32))
                elif self.local_aug_type in ['lla', 'bezier']:
...
def get_training(modality,location_scale, idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, local_aug_type='affine_clp'):
